...
namespace GameApp.BezierCurves
{
	...
	public class BezierCurve : MonoBehaviour
	{
		...
		{
			var tempArray = new List<BezierPoint>(points) {point};
		...
		{
			var tempArray = new List<BezierPoint>(points);
		...
		public Vector3 GetPointAt(float t)
		{
			...
			if (t >= 1) return points[points.Length - 1].position;
			...
			for (var i = 0; i < points.Length - 1; i++)
			{
				curvePercent = ApproximateLength(points[i], points[i + 1]) / length;
		...
		public static Vector3 GetPoint(BezierPoint p1, BezierPoint p2, float t)
		{
			...
			{
				return p2.handle1 != Vector3.zero
					? GetCubicCurvePoint(p1.position, p1.globalHandle2, p2.globalHandle1, p2.position, t)
					: GetQuadraticCurvePoint(p1.position, p1.globalHandle2, p2.position, t);
			}

			return p2.handle1 != Vector3.zero
				? GetQuadraticCurvePoint(p1.position, p2.globalHandle1, p2.position, t)
				: GetLinearPoint(p1.position, p2.position, t);
		}
