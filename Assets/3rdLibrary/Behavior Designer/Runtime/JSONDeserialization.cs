...
namespace BehaviorDesigner.Runtime
{
    public class JSONDeserialization : UnityEngine.Object
    {
        ...
        private static SharedVariable DeserializeSharedVariable(Dictionary<string, object> dict, IVariableSource variableSource, bool fromSource, List<UnityEngine.Object> unityObjects)
        {
            ...
            if (!fromSource && variableSource != null && dict.TryGetValue("Name", out nameObj) && (BehaviorManager.IsPlaying || !dict.ContainsKey("IsDynamic"))) {
                object globalObj;
...
#if UNITY_EDITOR
                if (dict.TryGetValue("Tooltip", out dictValue)) {
                    sharedVariable.Tooltip = dictValue as string;
                }
#endif
                            ...
                            if (isIDList) {
                                ...
                                if (fields[i].FieldType.IsArray) {
                                    ...
                                    for (int j = 0; j < objectValues.Count; ++j) {
                                        ...
                                        var element = ValueToObject(task, type, objectValues[j], variableSource, unityObjects);
                                        if (!type.IsInstanceOfType(element)) {
                                            objectArray.SetValue(null, j);
                                            continue;
                                        }
                                        objectArray.SetValue(element, j);
                                    }
        ...
        {
            var stringSplit = vector2String.Substring(1, vector2String.Length - 2).Split(',');
            return new Vector2Int(int.Parse(stringSplit[0], CultureInfo.InvariantCulture), int.Parse(stringSplit[1], CultureInfo.InvariantCulture));
        }
        ...
        {
            var stringSplit = vector3String.Substring(1, vector3String.Length - 2).Split(',');
            return new Vector3Int(int.Parse(stringSplit[0], CultureInfo.InvariantCulture), int.Parse(stringSplit[1], CultureInfo.InvariantCulture), int.Parse(stringSplit[2], CultureInfo.InvariantCulture));
        }
