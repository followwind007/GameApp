...
#pragma warning disable 219
        ...
        var unknownTask = new BehaviorDesigner.Runtime.Tasks.UnknownTask();
...
namespace BehaviorDesigner.Runtime
{
    ...
    public class BehaviorManager : MonoBehaviour
    {
...
#if !DLL_RELEASE
        public bool UnitTesting { set { unitTesting = value; } }
        private bool unitTesting;
#endif

        public delegate void BehaviorManagerHandler();

        public BehaviorManagerHandler OnEnableBehavior { get { return onEnableBehavior; } set { onEnableBehavior = value; } }
        ...
        // Third party support
        public enum ThirdPartyObjectType { PlayMaker, uScript, DialogueSystem, uSequencer }
        ...
        public void EnableBehavior(Behavior behavior)
        {
            ...
            if (behavior.AsynchronousLoad) {
                BehaviorThreadLoader loader = new BehaviorThreadLoader(behavior, LoadBehavior);
                Thread thread = new Thread(new ThreadStart(loader.LoadBehavior));
                loader.Thread = thread;
                thread.Start();
                // Keep track of the thread so it can be completed on the main thread.
            ...
            while (activeThreads.Count > 0) {
                for (int i = activeThreads.Count - 1; i >= 0; --i) {
                    if (!activeThreads[i].Thread.IsAlive) {
                        LoadBehaviorComplete(activeThreads[i].Behavior, activeThreads[i].BehaviorTree);
        ...
        private BehaviorTree LoadBehavior(Behavior behavior, GameObject behaviorGameObject, string gameObjectName, Transform behaviorTransform)
        {
            ...
            var behaviorTree = ObjectPool.Get<BehaviorTree>();
            lock (lockObject) {
            ...
            var status = AddToTaskList(behaviorTree, rootTask, behaviorGameObject, behaviorTransform, ref hasExternalBehavior, taskAddData);
            if (status < 0) {
                // something is wrong with the tree. Don't go any further
                switch (status) {
                    case -1:
                        behaviorTree.errorState = string.Format("The behavior \"{0}\" on GameObject \"{1}\" contains a parent task ({2} (index {3})) with no children. This behavior will be disabled.",
                            behavior.GetBehaviorSource().behaviorName, gameObjectName, taskAddData.errorTaskName, taskAddData.errorTask);
                        break;
                    case -2:
                        ...
                        break;
                    case -3:
                        behaviorTree.errorState = string.Format("The behavior \"{0}\" on GameObject \"{1}\" contains a null task (referenced from parent task {2} (index {3})). This behavior will be disabled.",
                                behavior.GetBehaviorSource().behaviorName, gameObjectName, taskAddData.errorTaskName, taskAddData.errorTask);
                        break;
                    case -4:
                        ...
                        break;
                    case -5:
                        behaviorTree.errorState = string.Format("The behavior \"{0}\" on GameObject \"{1}\" contains a Behavior Tree Reference task ({2} (index {3})) that which has an element with a null value in the externalBehaviors array. This behavior will be disabled.",
                                behavior.GetBehaviorSource().behaviorName, gameObjectName, taskAddData.errorTaskName, taskAddData.errorTask);
                        break;
                    case -6:
                        behaviorTree.errorState = string.Format("The behavior \"{0}\" on GameObject \"{1}\" contains a root task which is disabled. This behavior will be disabled.",
                            behavior.GetBehaviorSource().behaviorName, gameObjectName);
                        break;
                }
            }
            taskAddData.Destroy();
            ObjectPool.Return<TaskAddData>(taskAddData);
            ...
            if (task is BehaviorReference) {
                        ...
                        if (externalRootTask != null) {
                            ...
                            if (behaviorReference.variables != null) {
                                for (int j = 0; j < behaviorReference.variables.Length; ++j) {
                                    if (behaviorReference.variables[j] == null) {
                                        continue;
                                    }
                                    ...
                                    if (!data.overrideFields.ContainsKey(behaviorReference.variables[j].Value.name)) {
                                        ...
                                        if (behaviorReference.variables[j].Value is NamedVariable) {
                                            ...
                                            if (namedVariable.value != null) {
                                                ...
                                                } else if (!string.IsNullOrEmpty(namedVariable.value.Name) && data.overrideFields.TryGetValue(namedVariable.value.Name, out namedOverrideFieldValue)) {
                                                    overrideFieldValue = namedOverrideFieldValue;
                                                }
                                            }
                            ...
                            if (loadedBehaviorSource.Variables != null) {
                                for (int j = 0; j < loadedBehaviorSource.Variables.Count; ++j) {
                                    if (loadedBehaviorSource.Variables[j] == null) {
                                        continue;
                                    }
                            ...
                            int status = 0;
                            if ((status = AddToTaskList(behaviorTree, externalRootTask, behaviorGameObject, behaviorTransform, ref hasExternalBehavior, data)) < 0) {
                                return status;
                            }
                ...
                if (task is ParentTask) {
                    ...
                    for (int i = 0; i < childCount; ++i) {
                        ...
                        behaviorTree.parentCompositeIndex.Add(data.compositeParentIndex);
                        if ((status = AddToTaskList(behaviorTree, parentTask.Children[i], behaviorGameObject, behaviorTransform, ref hasExternalBehavior, data)) < 0) {
        ...
        public void DisableBehavior(Behavior behavior, bool paused, TaskStatus executionStatus)
        {
            if (!IsBehaviorEnabled(behavior)) {
                ...
                if (pausedBehaviorTrees.ContainsKey(behavior) && !paused) {
                    ...
                    // The behavior tree may be loading.
        ...
        private void Tick(BehaviorTree behaviorTree)
        {
            ...
            for (int i = 0; i < initialStackCount; ++i) {
                ...
                int interruptedTask;
                if (i < behaviorTree.interruptionIndex.Count && (interruptedTask = behaviorTree.interruptionIndex[i]) != -1) {
                    behaviorTree.interruptionIndex[i] = -1;
                    while (behaviorTree.activeStack[i].Peek() != interruptedTask) {
                        int stackCount = behaviorTree.activeStack[i].Count;
                        PopTask(behaviorTree, behaviorTree.activeStack[i].Peek(), i, ref status, true);
                        if (stackCount == 1) {
                            break;
                        }
                    }
                    ...
                    if (i < behaviorTree.activeStack.Count && behaviorTree.activeStack[i].Count > 0 && behaviorTree.taskList[interruptedTask] == behaviorTree.taskList[behaviorTree.activeStack[i].Peek()]) {
                        ...
                        PopTask(behaviorTree, interruptedTask, i, ref status, true);
                    }
                }
                int startIndex = -1;
                int taskIndex;
                while (status != TaskStatus.Running && i < behaviorTree.activeStack.Count && behaviorTree.activeStack[i].Count > 0) {
                    ...
                    if ((i < behaviorTree.activeStack.Count && behaviorTree.activeStack[i].Count > 0 && startIndex == behaviorTree.activeStack[i].Peek()) || !IsBehaviorEnabled(behaviorTree.behavior)) {
                        break;
                    } else {
                        startIndex = taskIndex;
                    }
                    status = RunTask(behaviorTree, taskIndex, i, status);
                }
            }
        }
            ...
            for (int i = behaviorTree.parentReevaluate.Count - 1; i > -1; --i) {
                ...
                if (behaviorTree.taskList[parentReevaluateIndex] is Decorator) {
                    if (behaviorTree.taskList[parentReevaluateIndex].OnUpdate() == TaskStatus.Failure) {
                        Interrupt(behaviorTree.behavior, behaviorTree.taskList[parentReevaluateIndex], TaskStatus.Inactive);
                    }
        ...
        private TaskStatus RunParentTask(BehaviorTree behaviorTree, int taskIndex, ref int stackIndex, TaskStatus status)
        {
            ...
            if (!parentTask.CanRunParallelChildren() || parentTask.OverrideStatus(TaskStatus.Running) != TaskStatus.Running) {
                ...
                while (parentTask.CanExecute() && (childStatus != TaskStatus.Running || parentTask.CanRunParallelChildren()) && IsBehaviorEnabled(activeBehavior)) {
                    if (behaviorTree.childrenIndex.Count <= taskIndex) {
                        break;
                    }
                    ...
                    if (behaviorTree.activeStack[i].Count > 0) {
                        ...
                        while (taskIndex != -1) {
                            if (taskIndex == interruptionIndex) {
                                ...
                                behaviorTree.interruptionTaskStatus[i] = interruptTaskStatus;
#if UNITY_EDITOR || DLL_DEBUG || DLL_RELEASE
                                if (behavior.LogTaskChanges) {
